...
export function CustomersTab({ companyId }: { companyId: string }) {
  ...
  const table = applyTable(
    ...
    (c) =>
      ...
      (c.protheusCode ?? '').toLowerCase().includes(q),
    sort,
  ...
  const columns: Column<Customer>[] = [
    ...
    },
    {
      key: 'document',
      header: 'Documento',
      render: (c) => <span className="text-[var(--text-muted)]">{c.document ?? '—'}</span>,
    },
    {
      key: 'email',
      header: 'E-mail',
      render: (c) => <span className="text-[var(--text-muted)]">{c.email ?? '—'}</span>,
    },
    {
      key: 'phone',
      header: 'Telefone',
      render: (c) => <span className="text-[var(--text-muted)]">{c.phone ?? '—'}</span>,
    },
    {
      key: 'code',
    ...
    { key: 'status', header: 'Status', render: (c) => <StatusBadge active={c.active} /> },
    {
      ...
      render: (c) => (
        <ActionMenu
          ...
          onToggle={() =>
            toggleEntity.mutate({ entity: 'customers', entityId: c.id, active: !c.active })
          }
        />
      ),
    },
        ...
        <CustomerModal
          ...
          onSaved={() => {
            ...
            queryClient.invalidateQueries({
              queryKey: companiesKeys.entity(companyId, 'customers'),
            })
          }}
        />
      )}
