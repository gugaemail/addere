import type { Order, OrderItem, Product, Customer, Branch, User, Transportadora, CondPag } from '@prisma/client'
...
export function buildOrderPayload(order: OrderWithRelations) {
  ...
  if (!order.user.idVendProt)       throw unprocessable('Vendedor sem código Protheus configurado (idVendProt)')

  const emissaoStr = formatDateDDMMYYYY(order.emissao ?? new Date())

  const itens = order.items.map((item) => {
    if (!item.product.protheusCode) throw unprocessable(`Produto "${item.product.name}" sem código Protheus configurado`)
