...
export default async function syncRoutes(app: FastifyInstance) {
  ...
  const syncEndpoints: Array<{ path: string; label: string; run: (companyId: string) => Promise<unknown> }> = [
  ...
  for (const endpoint of syncEndpoints) {
    app.post(endpoint.path, { preHandler: syncPermission, config: rateLimited }, async (request, reply) => {
      ...
      try {
        ...
        return reply.status(502).send({ message: err instanceof Error ? err.message : 'Erro desconhecido' })
      }
    })
  }
  ...
  function registerEntityTest(opts: {
    ...
    buildBody: (company: Company & { branches?: { idProtheus: string | null }[] }) => Record<string, unknown>
    includeBranches?: boolean
  }) {
    app.post(opts.path, { preHandler: syncPermission }, async (request, reply) => {
      const parsed = companyIdSchema.safeParse(request.body)
      if (!parsed.success) {
        return reply.status(400).send({ message: 'companyId inválido', errors: parsed.error.flatten() })
      }
      ...
      const requestBody = opts.buildBody(company as Company & { branches?: { idProtheus: string | null }[] })
      ...
      try {
        ...
        return reply.send({ ok: false, step: opts.step, error: e.message, status: e.response?.status })
      }
    })
  }
  ...
  app.post('/test-order/:id', { preHandler: [syncPermission, requireCompany] }, async (request, reply) => {
