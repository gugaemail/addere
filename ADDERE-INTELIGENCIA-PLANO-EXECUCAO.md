...
- v0.4 → v0.5: **payload real do consultor (20/08/2026)** fixou o contrato §2.7 — campo `query`; resposta `{success, page, pageSize, count, hasNext, columns[{name,type}], items[]}` com `items` como objetos; paginação `page`/`pageSize` guiada por `hasNext` (não `limite`/`deslocamento`). Adapter E2 alinhado (19ca15e); payload de erro `{success:false, errorId, message}` confirmado (285ebbc); paginação por query string `?page&pageSize` + URL do endpoint `/rest/WSQUERY` confirmadas por print (bb6c572). **Contrato E0-1 fechado sem pendências** — resta só confirmar se a URL fornecida é homolog ou produção.
...
- **P3** Resposta ✅ **confirmada pelo payload real**: `{ success, page, pageSize, count, hasNext, columns: [{name, type: 'C'|'N'|…}], items: [...] }` — `items` já vem como **array de objetos** `{coluna: valor}` (números como JSON number; tipo Protheus por coluna em `columns[].type`; aliases podem ser numéricos, ex.: `"202601"`). O `ProtheusSqlAdapter` consome `items` direto e mantém o fallback colunar/nomes de campos configuráveis por `syncConfig.sqlApi`. Payload de erro também confirmado (ver P8).
- **P4** Paginação ✅ **confirmada pelo print do consultor** (20/08/2026): `page`/`pageSize` na **query string** (`POST <apiSql>?page=1&pageSize=100`), body só com `{query}`; a resposta ecoa `page`/`pageSize` e traz `hasNext` (**sem total geral**). O adapter monta a query string preservando params existentes da URL e pagina enquanto `hasNext=true` (nomes configuráveis via `syncConfig.sqlApi`).
...
- **P8** Erro de negócio ✅ **confirmado pelo payload real** (20/08/2026): `{ success: false, errorId: 'WSQ002', message: 'Somente comandos SELECT (ou WITH ... SELECT) sao permitidos' }` — o adapter propaga `[errorId] message` truncada em 200 chars; erros de transporte seguem HTTP 4xx/5xx via `protheusPost` (sanitizar antes de exibir na prévia). Bônus: a mensagem do WSQ002 confirma que o endpoint aceita **CTE (`WITH ... SELECT`)** — coerente com a decisão D6.
- **P9** **Existe homologação** → empresa piloto entra no Addere staging apontando para homologação; troca para produção no fim do onboarding. URL do endpoint fornecida pelo consultor em 20/08/2026 (path `/rest/WSQUERY`, host DDNS http na porta 8088 — passa no anti-SSRF, que só bloqueia IP privado/loopback). **Cadastrar em `Company.apiSql` pelo painel, não versionar a URL no repo**; confirmar com o consultor se essa URL é homologação ou produção.
...
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | -------------------------------------------- |
| 1   | ~~Contrato da API "qualquer SELECT"~~ ✅ **FECHADO** (P1–P9 + payloads/print reais no §2.7; campo `query`, resposta, erro `errorId/message` e paginação `?page&pageSize` confirmados; adapter alinhado até bb6c572). Resta: confirmar se a URL é homolog ou produção | E4 (validar no backfill real) | Gustavo + consultor Protheus                 |
